...
const selectItem = (event, img) => {
  ...
  element.classList.toggle('added');//Selected and deselected maker
 
  let item = sliders.indexOf(img);
  if (item === -1) {
    sliders.push(img);
  } else {
    let deselect = sliders.indexOf(img);//Deselect selected one
...
sliderBtn.addEventListener('click',
  function () {
    const duration = document.getElementById('duration').value || 1000;
    if (duration < 0) {
        alert('Negative time!!!! ⚠.Please Enter the positive value');
    } else {
        createSlider();
    }
});
